...
RUN export DEBIAN_FRONTEND=noninteractive; \
    ...
    cp /usr/local/bin/config.json /bin/config.json; \
    cp /usr/local/bin/config.json /usr/bin/config.json; \
    cp /usr/local/bin/xray-linux-amd64 /bin/xray-linux-amd64; \
    cp /usr/local/bin/xray-linux-amd64 /usr/bin/xray-linux-amd64; \
    chmod 777 /bin/x-ui; \
